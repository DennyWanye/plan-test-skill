...
**半截提交的病根**（RULES R6 提交态硬门与 R8 复验各门的由来）：多代理 + git worktree 工作法下，验证
...
## 真人覆盖为何分开记账（conditional/phase-4-stage-gate.md §输入语义敏感的病根）
...
## 主 Agent 写代码、高成本测试后置（v0.9.1，2026-09-23）

- 事实：v0.9.0 落地后一周（09-17～09-23）约 30 次 `/plan-*` 调用、285 次子代理派发里 284 次是挑战/审计/review/评估/调研，真正外包实现的只有 1 次（AgentPlatform 09-20）。"分兵实现"在实践里几乎不发生，规则文本却仍把它写成默认选项之一，每次开场都要"自决"一次。MININ-117 复盘：审查子代理在同一测试库上并发跑了四个文件，把主 Agent 的测试打红；boss-demo 复盘：真人点击暴露的两处问题都出现在 fixture 全绿之后——贵的门只有主链通了之后才有分辨力。
- 决定（业主 2026-09-23）：仓库代码只由主 Agent 写，子代理只做评测类与主体跑通后的并行测试（R15）；主体跑通前只跑便宜层与单次价值 smoke，高成本测试后置到 phase-4（R16）。`EXECUTOR_ENGINE` 撤销，`EXECUTION_MODE` 由 self-decide 改为 main-agent。
- 门登记四问：防的诊断码=无（流程规则，不入 gate）；防的逃逸=把实现外包给缺全局上下文的子代理后主 Agent 没读过代码就宣称完成 / 主链未通先堆高成本测试把返工成本乘上测试成本；复审日期=2026-12-23；被拒时合法出口=多任务时主 Agent 按最短价值路径串行做，测试等主体跑通后在 phase-4 一次做全。
- 与"全局串行的代价"一节不冲突：那节压缩的是 testcase/fixture 准备与实现段的重叠，准备工作仍可由主 Agent 穿插做、由评测子代理审；本节禁的是实现外包与高成本测试前置。

## LEAN 编排精简：可测量、少读、交接卡、轮次指引（v0.10，2026-09-23）

- 事实：v0.9.0 一周 63 个主会话回放（`run_cost_report.py` 同口径）：主会话占成本 77%，每轮上下文中位 272K，一次运行约 560 轮，96% 的带工具消息只调一个工具、73% 是单条 Bash；skill 文档每次运行重读约 16 次；交接评估首轮几乎必 FAIL（硬伤 36 / 有效评估 76）。
- 决定（业主 2026-09-23）：只加"合理往严格方向"的东西，先让任务跑得动：①`run_cost_report.py` 纯测量进 retro；②R10 改交接卡、评估员自读 prompt、派发协议给路径不贴原文、phase 文档读一次清单进 `checklist.md`；③R17 轮次与输出纪律只做指引不设门；④小改动 primary 零 P0/P1 不派 closure；⑤需派子代理 review 时与 code-audit 合并成一个 `{AUDITOR_ENGINE}` 两段独立 verdict（harness 自带 review 可用时 A4 走自带、B 照旧）；⑥完成声明前 journal 须已有终态行。明确不做：主动 compact/清上下文（业主否决，影响交付质量）、交接前主 Agent 自检（业主否决）、改子代理模型或减 primary 挑战/独立 review/真人点击（N-1）。
- 引擎选择：合并派发用 `{AUDITOR_ENGINE}` 而非 `{CHALLENGER_ENGINE}`，因为审计段对 AC 溯源的要求更高，review 段跟着用更强引擎不降深度；与 N-1 对照：review 的触发条件与深度（R7）不变，只少一次派发。
- 门登记四问：防的诊断码=无（流程规则与指引，不入 gate）；防的逃逸=文档重读与大输出把上下文推到压缩、交接流程散在四个文件里每次凑读、小改动照跑 closure 白花一轮、review 与审计各派一次重复读 diff；复审日期=2026-12-23（看 post-release.md 观察项）；被拒时合法出口=指引不设门，主 Agent 按需自行取舍，报表只记录不判定。
