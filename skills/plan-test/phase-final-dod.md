...
4. **自我批评一行**（`SELF_CRITICISM = required`）：回答"本次哪些门空转了、哪里被仪式拖慢了、
   ...
   数据从这里来。
5. **提交**：全部改动提交，工作树干净。
5b. **交接复核（`HANDOFF_CHECK`）**：在提交与第 6 步 push 前 review 之后、把完成消息发给用户之前——
   按 `checklists/handoff.md` H1 拿交付的真实入口逐条对照原话对照表，H0 写清消息结构，
   再派评估员（`MODE: full`）；硬伤修好重评 PASS 才发，文字类改完即发。评估记录行写进 journal。
...
- [ ] **整体可用性实测通过**：按原始需求的核心使用路径整机走通，含每个新增可交互元素的可见反馈、等待/空/失败态的出路、至少一条真实数据路径（判据见 `checklists/handoff.md` H2.2–H2.3）—— 证据：journal / 审计记录
...
- [ ] **最近一次交接评估 PASS 或 DISPUTED 且已记录**（`HANDOFF_CHECK`）—— 证据：journal 交接评估记录行 + 评估输出文件
...
升级时按 `checklists/handoff.md` H0/H3 组织消息（第一段先写需要用户做什么），并按 `references/user-attention.md` 说明已调查事实、已尝试方法、推荐出口和需要的最小用户动作；该消息走轻量评估。范围内仍能修复就继续；不得把 BLOCKED 自动作结束任务或要求重复批准的理由。
