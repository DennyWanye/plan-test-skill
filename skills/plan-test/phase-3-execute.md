...
3. **里程碑 PASS 后的两个强制动作**：
   - **到群众中去（demo）**：把跑起来的实物给用户看一眼——一个 URL、一张截图、一条可复制的
     命令，配一句用户语言的进度汇报。**demo 给了可操作入口就是交接**：发出前按
     `checklists/handoff.md` 走完整评估（H2 的真点、反馈、失败态证据对准本片已声明能力，
     未做的部分在消息里写明）。让用户在半成品上纠偏，比 plan 文字批准防返工能力强得多。
     demo 默认异步展示，不等待用户确认；用户要求先验收或出现重要行为偏差时，按 `references/user-attention.md` 处理。
     汇报以**原始 plan 总进度表**开头（config `PROGRESS_REPORTING`）；同一时刻要用户拍板的事项按
     config `DECISION_BATCHING` 攒成一批、附默认建议一次问完，不许一条一条追着要批准。
...
3. **findings 处理**：
   - P0/P1（正确性缺陷）：必须修复；**每个修复配一条决定性测试**（能证明该 bug 真修了的
     ...
     **决定性测试必须是可复跑的自动化断言、真的落进回归套件文件**——手工验证/一次性
     spike/scratchpad 实测都不算（病根：2026-09-01 v0.7.1 把手工实测当决定性测试，
     155 行生产代码零新增用例，次日被审计抓包）；
   - P2 及以下（风格/优化建议）：记入 journal 遗留清单，不阻塞 review 闭环；
     其中**用户可见**的问题（文案、报错提示、交互反馈）在交用户验收前按 `checklists/handoff.md` H2.4 清零；
