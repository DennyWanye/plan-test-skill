...
## R3 oracle 先于实现与冻结
- **oracle 先于实现**（`ORACLE_FREEZE`）— 触发时点：写任一 AC 实现前；要求：先写"什么算对"（输入、独立预期、通过条件）进验证栏/testcase 草稿，选中用例冻结 revision 本 run 执行；防的逃逸：照实现补预期。
...
## R15 主 Agent 写代码，子代理只评测
- **子代理分工**（`SUBAGENT_SCOPE`=eval-and-test-only、`EXECUTION_MODE`=main-agent）— 触发时点：任何阶段派子代理前；要求：仓库代码（业务/接线/测试代码/脚本/配置/迁移）只由主 Agent 亲手写、改、修，不派子代理实现任务、修 finding、写测试代码或"顺手改"；子代理只做①评测类：plan 挑战/完成度审计/code review/最小化审查/交接评估/测试结果评估/testcase 迭代/只读调研，②测试类：跑主 Agent 已写好的可并行测试（时点与隔离见 R16）；子代理输出只有 findings/verdict/证据，应用与修复回到主 Agent；防的逃逸：把实现外包给没有全局上下文的子代理，主 Agent 没读过代码就宣称完成。
- 子代理 prompt 首段写明"只读、不改仓库文件、不 commit"；评测型子代理不跑会与主 Agent 互相干扰的测试（共享测试库/端口）。spike 即弃不入库不算开发。集中兵力原则不变：多任务也是主 Agent 按最短价值路径串行，并行只用于评测与测试。
