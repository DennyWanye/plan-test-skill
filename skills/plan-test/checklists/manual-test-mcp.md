...
**处置（二选一，写进 phase-4 ③3 兑现表）**：要么**由用户亲自补测至少 1 个核心场景**（安排在交接评估 PASS 之后，作为用户验收）；要么在 AI 驾驶中**人为注入**这些熵——改设备日期/周期标记模拟翻转、`force-stop`+清数据后冷启动（注意：仅杀进程重进是暖重启，不算）、长会话压测（≥10 轮历史后再执行核心操作）、token 置过期后重登。哪类熵没注入、为什么，记入表注。

## 工具选择（MCP_DRIVER = auto 时按此选）

| 被测对象 | 工具 | 说明 |
|----------|------|------|
| Web 应用 | **harness 内置浏览器**（如 Claude Code 桌面版 Browser 面板）首选，其次 **Claude-in-Chrome MCP**（`mcp__claude-in-chrome__*`） | DOM 感知，比像素点击快且稳。 |
| 原生桌面应用 | **computer-use / macos-mcp** | 真鼠标键盘。 |
| 有专用 MCP 的应用（Slack/Gmail 等） | 该应用的专用 MCP | API 级，最快最准。 |

## 权限 tier 注意（容易撞墙，先看清）

- **浏览器在 computer-use 下是只读 tier**：能截图看，但点击/输入被拦。要操作浏览器用 **harness 内置浏览器或 Claude-in-Chrome MCP**。
...
0. **禁止代点**：不得用 `javascript_tool`、`$wire.set`、`dispatchEvent`、直接调接口代替真实点击与输入；
   JS 只能读状态或造前置数据。代点得到的 PASS 不作数（`scripts/handoff_evidence.py` 会从会话记录里把它认出来）。
   确实点不了 → BLOCKED，按 `checklists/handoff.md` H3 请用户批准等价方案；"如实说明没真点"不是出口。
...
**计数纪律**（见 phase-A 场景矩阵与 config `MANUAL_MIN_DISTINCT_CLASSES`）：同一 `input_class` + 归一化后相同意图的输入（含改写）只记 retry/replay，不得记为新 root 场景；"换领域/难度/风险形态"才算新场景。

## 测试后

- 全部 testcase 通过后**不再单独派终审子代理**：由交接评估（`HANDOFF_CHECK`）的表 2 核对是否真按 testcase 跑完——
  派发时把 testcase 路径与逐步执行记录一并给评估员，漏的补齐后按 `fix_class` 处理。
- 清理 / 隔离测试数据，避免污染。
