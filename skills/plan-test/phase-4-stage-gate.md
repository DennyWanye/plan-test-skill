...
6. **全表面冒烟（`FULL_SURFACE_SMOKE = required`）**：对**每一个用户可达的端点/入口**各打最小一枪（一条 curl 或一次 MCP 点击），断言非 404 / 非 500 / 非"未接通"降级码。范围是**全部历史端点**，不只本次改动链路——核心价值 smoke 验的是"这次改的那条链路"，照不出"别的功能被静默弄坏/从没接通"。**判 FAIL 规则：任一历史端点 404 / 未接通降级 → Gate FAIL；"这次没动它"不是借口**——它可能被别的改动或半截提交弄坏。枚举型参数（如 persona 白名单）要**逐值各打一枪**，最容易漏接的就是新加的那个值。冒烟脚本存盘、可复跑、纳入回归套件（会话续接时的第一道复验也用它，见 SKILL.md 推进规则）。
