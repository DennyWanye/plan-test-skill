...
按 `references/delivery-slices.md` 检查整体 AC/风险与当前片：是否把数据库、后端、UI 横向半成品冒充交付；当前累计产物是否有真实使用入口与独立预期；是否只有 mock/内部测试；后续片不做当前是否仍有用途；是否遗漏保留能力与最终跨片组合验证；整体成败假设是否被推迟。小需求单片、SDK真实消费者样例均可成立。未来片尚未细化不是缺陷，遗漏整体风险或进入未就绪片才是。发现需绑定具体 AC/风险，不为片数或格式凑 P0。
