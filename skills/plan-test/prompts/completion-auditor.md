...
6. **整体可用性**：站在真实用户视角走一遍原始需求的核心使用路径——这个功能**整体现在能用吗**？零件各自通过 ≠ 整机可用；组合起来走不通 → 整体判 FAIL，指出断在哪一环。
7. **补丁掩盖检查**：有没有"任务标完成，但实现方式是 hack/特例分支/绕行"的痕迹？这类补丁能让任务打勾却让对应 AC 名存实亡——列出来，标注"疑似 plan 层缺陷被补丁掩盖"。
...
## 主要矛盾判定（最先给出）
- 主要矛盾对应 AC：AC-x；是否真达成（业务可用）：是/否 + 证据
