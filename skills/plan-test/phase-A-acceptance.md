...
1c. **用户原话与原话对照**（`checklists/handoff.md` H1）：把用户原话（需求文件、卡片、聊天原文）照抄进 acceptance 的「用户原话」节，再按句拆成最小条目写进「原话对照」表：`原话条目（原文照抄） | 对应 AC | 落在哪个界面/入口 | 状态`。每条必须对应到 AC；对应不上只有两种合法状态——`用户已批准不做/缩减（附原话出处）`、`待用户决定（进决策批次）`。**自己判断"这条不重要/以后再做"而不问 = 悄悄做少**。原话写了"在哪里"（如"详情页支持…"），AC 必须保留这个位置。质量类要求（简洁明了、看得懂、说明原因）在宣布完成前要用截图对照，不能只凭代码判兑现。
