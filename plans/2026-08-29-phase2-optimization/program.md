...
## 2b. 执行结果（2026-08-29 收尾回写）

**24 项：21 项完成，3 项显式未做/推迟（决策 C/D 授权，逐条留痕）。**
每波一批 commit（W1=c1bfb65 W2=c017ab6 W3=8a2739c W4=fcd0be1 W5=f98032c W6=b26ab09），
每波全套测试真 rc 校验；终态 **333 项全绿**（起点 303）。诊断码 53→56（+4 新 −3 退休 +3 新）。

**未做清单（不静默消失）**：
| 项 | 处置 | 依据 |
|---|---|---|
| W3-13 acceptance/contract 替换独立命令 | 未做 | 核心危害被 W3-12 快照区分吸收（换约必付全量复核）；独立命令留下批 |
| W5-19 轮次门退休评审 | 推迟 | 评审数据源（间隔分布）已就绪；评审本身等发布后攒真实数据，不预设结论（决策 C） |
| W6-21 phase-2 热冷分离 | 收窄 | 实测仅一行内联注、无大段病根可迁——原假设偏高；phase-4 部分已完成 |
| flow_tier evidence_basis 机器复核（no_deletions 等断言实跑验证） | 未做 | 层 1（入账+交叉校验）已落地；层 2 留待 refusal/遥测数据攒够后一并设计 |

**已知需业主动作**：发布（bump 版本 + 两侧 plugin update）——按决策 C 等业主发话；
发布前 refusal/遥测/间隔分布均只在源码仓库生效，日常使用不积累数据。
