...
> **⏭️ 第五轮（2026-08-28，审计与上溯）见 [`AUDIT-2026-08-28-gate-authority.md`](./AUDIT-2026-08-28-gate-authority.md)。**
> 那一轮复核了第四轮的转述数字（**修正两处**）并上溯到结构根因，**推翻了第四轮 §3.4 的
> 单点放宽方案**。两个关键发现：① 挑战循环与 receipt 是**不相交的两个集合**（4 张 receipt
> 全发在没有挑战循环的账本上）；② `validate()` 根本不引用 `challenge_loops`，
> 三个 `LOOP_*` 诊断码无产生点——**挑战循环是一道纯自觉的门**。
> 结构根因：**记录职能与裁决职能寄生在同一次写入上**（`die()` 161 处调用点无一写账本）。

> **第四轮（2026-08-28，run log 实证驱动）见 [`HANDOFF-2026-08-28-runlog.md`](./HANDOFF-2026-08-28-runlog.md)。**
> 那一轮从两份真实日志（18 本账本 + 1888 次 gate 调用）里挖出**三条独立成因，都通向
> 「换个 run-dir 重来」**：粘性 fail、`LEDGER_TAMPERED` 误报、`CONTROL_NOT_REQUIRED`。
> 前两条已堵（`0a53b8b`），第三条已定位并给出方案——**第五轮建议改用普遍机制，勿按原方案实施。**
